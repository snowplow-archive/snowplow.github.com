...
1. Incoming custom unstructured events and contexts are parsed to ensure that they are valid JSON
2. Where possible, the JSON is cleaned up (e.g. whitespace removed)
3. The JSON is then stored as `json`-type fields in Postgres, and in large `varchar` fields in Redshift
...
* We have updated the StorageLoader so that it can handle `dvce_timestamp` values outside of the standard range
* We have updated fields containing UUIDs from `varchar(38)` to `char(36)`
* We have changed the `DISTKEY` for `atomic.events` in Redshift to be `event_id`, to facilitate JOINs coming in future Snowplow releases
...
<table class="table table-striped">
  ...
  <tbody>
    <tr>
      <td><strong>Redshift</strong></td><td>[0.3.0] [redshift-table]</td><td>[Migrate from 0.2.2] [redshift-migration]</td>
    </tr>
    <tr>
      <td><strong>Postgres</strong></td><td>[0.2.0] [postgres-table]</td><td>[Migrate from 0.1.x] [postgres-migration]</td>
    </tr>
  </tbody>
</table>
...
* Add support for validating your unstructured events and contexts against your JSON Schemas
* Automatically shred your unstructured events and contexts to dedicated Redshift and Postgres tables using [JSON Path] [json-path]
...
So a huge amount planned! We are super excited about Snowplow being the first open source analytics platform to make the leap into unstructured event analytics. Stay tuned for further updates on this - and if you would like to read up for what is coming soon, we would encourage checking out this [excellent guide to JSON Schema] [json-schema-guide] (PDF).
