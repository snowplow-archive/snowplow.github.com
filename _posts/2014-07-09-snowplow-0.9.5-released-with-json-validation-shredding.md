...
We are hugely excited to announce the release of Snowplow 0.9.5: the first event analytics system to validate incoming JSONs (using JSON Schema), and then automatically shred those JSONs into dedicated tables in Amazon Redshift.

![web-page-context-img] [web-page-context-img]
...
There are two great use cases for our new shredding functionality:
...
2. Defining your own custom unstructured events and contexts, and processing these through the Snowplow pipeline into Redshift
...
![shredding-architecture-img] [shredding-architecture-img]
...
<div class="html">
<h2><a name="new-types">1.2 New Snowplow event types</a></h2>
</div>
...
* A JSON Schema hosted in Iglu Central
* A Redshift table definition available in the Snowplow repo
...
You also need to update the `config.yml` file for EmrEtlRunner. You can find an example of this file in GitHub [here] [emretlrunner-config]. For more information on the new configuration file, see XXXXXXXXXXXXXXXXXXX.
...
You also need to update the `config.yml` file for EmrEtlRunner. You can find an examples of this file in GitHub:

* [Sample file for Postgres] [storageloader-pg-config]
* [Sample file for Redshift] [storageloader-red-config]

For more information on the new configuration file, see XXXXXXXXXXXXXXXXXXX.
