...
1. [POST support](/blog/2015/xx/x/snowplow-javascript-tracker-2.3.0-released/#post)
2. [Customizable form tracking](/blog/2015/xx/x/snowplow-javascript-tracker-2.3.0-released/#forms)
3. [Automatic contexts](/blog/2015/xx/x/snowplow-javascript-tracker-2.3.0-released/#contexts)
4. [Development quickstart](/blog/2015/xx/x/snowplow-javascript-tracker-2.3.0-released/#post)
5. [Other improvements](/blog/2015/xx/x/snowplow-javascript-tracker-2.3.0-released/#other)
6. [Upgrading](/blog/2015/xx/x/snowplow-javascript-tracker-2.3.0-released/#upgrading)
7. [Documentation and getting help](/blog/2015/xx/x/snowplow-javascript-tracker-2.3.0-released/#help)

<!--more-->

<h2><a name="post">1. POST support</a></h2>

Until now, the JavaScript Tracker has sent events to Snowplow in the querystring for GET requests. The recently released version 0.10.0 of the [Clojure Collector][clojure-collector] added [CORS][cors] support, so it can now accept POST requests from the browser. This has two advantages:

- Internet Explorer's [querystring limit][ie-limit] doesn't limit the size of POST requests
- It's possible to batch multiple events in a single request
...
This release adds an additional `config` argument to `enableFormTracking`, allowing you to choose which fields and which forms get tracked. The configuration object should have two elements: "forms" and "fields". The "forms" value specifies which forms on the page will be tracked; the "fields" value specifies which fields of the tracked forms will be tracked.
...
These optional contexts are now configured in the argmap when creating a new tracker. (Note that the `performanceTracking` argument to the `trackPageView` method has been removed.) Use the new `contexts` field to choose which contexts are used:
...
There is one backwards-incompatible change: the `trackPageView` method now takes only takes two arguments, `customTitle` and `context`, rather than three.
