...
They are caused by `OPTIONS` requests that are made when users of the Javascript tracker send data via `POST`. (It is necessary for the browser to send an `OPTIONS` request prior to issuing the `POST` request with the actual data - this is a CORS requirement.) Again, they can safely be ignored. Again, we can filter these out, with the bad rows generated by malicious bots, using the following query in Kibana:
...
Addressing this issue is essential, then. Fortunately, this should be straightforward: most likely we need to create a new version of the schema that allows for the two fields, and update the tracker code to send in a reference to the new schema version in the different self-describing JSONs.
...
[img3]: /assets/img/blog/2016/02/kibana3.png
[img4]: /assets/img/blog/2016/02/kibana4.png
