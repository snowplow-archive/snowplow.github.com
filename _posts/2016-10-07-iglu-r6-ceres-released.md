---
layout: post
title: Iglu 6 Ceres released with key significant updates to Igluctl
title-short: Iglu 6 Ceres released
...
However, if you lint the above schema with the increased severity level 2 it will fail because the schema underdetermines the associated Redshift table definition:
...
Now when we use Igluctl to genearte our Redshift DDL we can see that Igluctl has correctly set the corresponding Redshift column type to `smallint`:
...
However, users that wanted to publish schemas to S3 backed static server repos, or publish jsonpath files to S3 so they can be used to load event and context data into Redshift, had to use another tool to do so. (Most commonly Amazon's excellent [AWS CLI][aws-cli]).
...
1. [Ability to publish public schemas to the Iglu scala server repo][public-schemas]. This means that Igluctl can now publish schemas to snowplow-mini, which currently requires that the schemas stored on the local service are public rather than private.
2. Improved [Windows support](https://github.com/snowplow/iglu/issues/209) including [a bug fix](https://github.com/snowplow/iglu/issues/208). Now there is proper documentation for Windows uses getting started with Igluctl.
