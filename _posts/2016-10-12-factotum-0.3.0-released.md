...
1. [Improving the workflow when creating DAGs](/blog/2016/10/12/factotum-0.3.0-released#workflow)
2. [Improving job monitoring (using webhooks)](/blog/2016/10/12/factotum-0.3.0-released#webhooks)
3. [Behaviours on task failure](/blog/2016/10/12/factotum-0.3.0-released#taskfailure)
4. [Extras](/blog/2016/10/12/factotum-0.3.0-released#extras)
5. [Downloading and running Factotum](/blog/2016/10/12/factotum-0.3.0-released#install)
6. [Roadmap](/blog/2016/10/12/factotum-0.3.0-released#roadmap)
7. [Contributing](/blog/2016/10/12/factotum-0.3.0-released#contributing)
...
this schema and ensure the Factfile you're working on is valid, so as of version 0.3.0 we've introduced a built-in validation command. This
...
Data-pipelines typically run on clusters, with a job or part of a job being sourced to one or more machines (which may or may not be virtual). It won't necessarily be known
...
There's a number of ways to "bridge" applications which use traditional log files for cluster use, for example using NFS and a central "log store". However this solution isn't perfect, and
...
Many tools such as Airflow would at this point bundle in mysql or postgres and use that to store state over time. It makes technical sense to make this decision, but it what it does
for a organisation is create a new and opaque data silo. All this information is hidden away somewhere, and liable to change format between releases.
...
Updates are split into two different event types. The first is triggered when the state of the job changes, for example it's started or finished. 
The second is when the state of a specific task changes (for example, a task started or failed).
...
Task updates are described by `com.snowplowanalytics.factotum/task_update/jsonschema/1-0-0` events (available in iglu-central).
...
<h2 id="taskfailure">3. Behaviours on task failure</h2>

## Fail fast vs continue as far as possible

In previous releases of Factotum, when a task fails Factotum will stop processing your job as soon as possible. We call this behaviour "failing fast". Failing fast is
simple and predictable, however it often results in a lot of tasks that could have been run to not run at all.

That's why as of this release, we're switching to a different model. Factotum will now complete as many tasks as possible, with the tasks that depend on failing tasks
being the only ones which are skipped.

Here's a few diagrams cataloguing the difference in behaviour. On the left is the previous version(s) of Factotum, and on the right is version 0.3.0+. 
...
In trivial DAGs (as shown above) the behaviour between this version of Factotum and prior versions is the same.
...
A task that requests early DAG termination has always worked in the same way as failures (except that it's not a failure!). To keep things straightforward, we've also
altered how "no operations" work in version 0.3.0 to match the new way of handling task failures (shown [above](#taskfailure)). They will continue to
...
[job-update-schema]: https://raw.githubusercontent.com/snowplow/iglu-central/master/schemas/com.snowplowanalytics.factotum/job_update/jsonschema/1-0-0
