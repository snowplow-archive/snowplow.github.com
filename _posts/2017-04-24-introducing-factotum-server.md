...
It was important that the job server be 100% compatible with the job running behavior of Factotum CLI and specifically Factotum's factfile format.
...
* Whether the last run was a success or failure
...
Assuming that the steps [above](#install) have been followed, you now have Factotum Server running with a copy of the Factotum binary, along with an example factfile.
